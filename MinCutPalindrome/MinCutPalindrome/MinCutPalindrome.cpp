// MinCutPalindrome.cpp : Defines the entry point for the console application.
//
...
int minCut(vector<vector<int>> A) {
	...
	for (int i = 0; i < m; i++)
	{
		...
		for (int j = 0; j < n; j++) {
			...
			table[i][j] = A[i][j] + (table[i - 1][j] > table[i][j - 1] ? table[i - 1][j] : table[i][j - 1]);
		}
	}

	int x =  table[m - 1][n - 1] > 0 ? 0 : abs(table[m - 1][n - 1]) + 1;
